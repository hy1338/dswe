...
local function IsEnemy(character)
    ...
    if plr and plr ~= player then
        return true  -- 默认所有其他玩家为敌人
    end
    -- NPC 默认为敌人
    if character:FindFirstChildOfClass("Humanoid") and not plr then
        return true
    end
    return false
end
...
local function getClosestEnemy()
    ...
    if not VisualAimbot.Enabled then return nil end
    local mp = UserInputService:GetMouseLocation()
    local enemies = getAllEnemies()
    for _, enemy in ipairs(enemies) do
        local hum = enemy:FindFirstChildOfClass("Humanoid")
        local hd = enemy:FindFirstChild("Head")
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        for _, enemy in ipairs(enemies) do
            local hd = enemy:FindFirstChild("Head")
            local hm = enemy:FindFirstChildOfClass("Humanoid")
...
local function findNeck(character)
    ...
    if head then
        for _, desc in ipairs(character:GetDescendants()) do
            if desc:IsA("Motor6D") and desc.Part1 == head then
                return desc
            end
        end
    end
...
local function applyAntiAim()
    ...
    if AntiAim.Desync then
        if AntiAim.DesyncJitter then
            desyncAngle = AntiAim._jitterState and AntiAim.DesyncOffset or -AntiAim.DesyncOffset
            if AntiAim.DesyncJitterRange > 0 then
                desyncAngle = desyncAngle + (AntiAim._jitterState and AntiAim.DesyncJitterRange or -AntiAim.DesyncJitterRange)
            end
        else
...
local function cleanupAntiAim()
    ...
    if AntiAim._neckInstance and AntiAim._neckInstance.Parent then
        if AntiAim._originalNeckC0 then
            AntiAim._neckInstance.C0 = AntiAim._originalNeckC0
            restored = true
        end
    end
...
AntiAimGroup:AddToggle("AntiAimToggle", { Text = "启用反瞄准", Default = false, Callback = function(v) AntiAim.Enabled = v; if not v then cleanupAntiAim() end end })
...
AntiAimGroup:AddDropdown("AntiAimPitch", { Text = "俯仰角度", Values = {"无","下","上","零"}, Default = "无", Callback = function(v) AntiAim.Pitch = v end })
AntiAimGroup:AddDivider()
AntiAimGroup:AddLabel("偏航 (Yaw)")
AntiAimGroup:AddDropdown("AntiAimYaw", { Text = "偏航模式", Values = {"向后","向左","向右","抖动","旋转","偏移"}, Default = "向后", Callback = function(v) AntiAim.Yaw = v end })
...
AntiAimGroup:AddSlider("AntiAimDesyncOffset", { Text = "Desync偏移", Default = 30, Min = 5, Max = 58, Rounding = 0, Suffix = "°", Callback = function(v) AntiAim.DesyncOffset = v end })
...
local function spawnViewmodel(knife)
    ...
    -- 尝试应用手套（如果存在）
    pcall(function()
        local tg = RS.Assets.Weapons:FindFirstChild("T Glove")
        if tg then
            attachAsset(tg,"Left Arm","Left Arm","Glove",CFrame.new(0,0,-1.5))
            attachAsset(tg,"Right Arm","Right Arm","Glove",CFrame.new(0,0,-1.5))
        end
        local sleeves = RS.Assets.Sleeves:FindFirstChild("IDF")
        local ctg = RS.Assets.Weapons:FindFirstChild("CT Glove")
        if sleeves and ctg then
    ...
    for _, enemy in ipairs(enemies) do
        ...
        if hum and hum.Health>0 and root and head then
            ...
            if rootOn then
                ...
                esp.boxOutline.Visible=false; esp.box.Visible=false; esp.name.Visible=false; esp.distance.Visible=false; esp.healthOutline.Visible=false; esp.healthBar.Visible=false for _,l in pairs(esp.bones) do l.Visible=false end end
        end
    end
