...
local AntiAim = {
    ...
    _jitterState = false,
    _lastJitterTick = 0,
    _originalHipHeight = 2,
    _fakeLagTicks = 0,
    _fakeLagStored = {},
    _connection = nil,
    -- 新增：多层级强制系统
    _bodyGyro = nil,
    _originalRootCFrame = nil,
    _neckMotor = nil,
    _rootMotor = nil,
    _waistMotor = nil,
    _originalNeckC0 = nil,
    _originalNeckC1 = nil,
    _originalRootC0 = nil,
    _originalWaistC0 = nil,
    _lastChar = nil,
    _initialized = false,
    _forceMethod = "CFrame", -- "CFrame" | "BodyGyro" | "Both"
}
...
local function findNeckMotor6D(character)
    ...
    if torso then
        for _, child in ipairs(torso:GetChildren()) do
            if child:IsA("Motor6D") and (child.Part1 == head) then return child end
        end
    end
    for _, obj in ipairs(character:GetDescendants()) do
        if obj:IsA("Motor6D") and obj.Name:lower():find("neck") then return obj end
    end
    return nil
end

-- 查找 Root/Hip Motor6D (连接 HRP 和 Torso 的关节)
local function findRootMotor6D(character)
    if not character then return nil end
    local hrp = character:FindFirstChild("HumanoidRootPart")
    if not hrp then return nil end
    for _, obj in ipairs(hrp:GetChildren()) do
        if obj:IsA("Motor6D") then return obj end
    end
    for _, obj in ipairs(character:GetDescendants()) do
        if obj:IsA("Motor6D") and (obj.Name:lower():find("root") or obj.Name:lower():find("hip") or obj.Name:lower():find("waist")) then
            if obj.Part0 == hrp or obj.Part1 == hrp then return obj end
        end
    end
    return nil
end

-- 查找 Waist Motor6D (R15: UpperTorso-LowerTorso 连接)
local function findWaistMotor6D(character)
    if not character then return nil end
    local upperTorso = character:FindFirstChild("UpperTorso")
    local lowerTorso = character:FindFirstChild("LowerTorso")
    if upperTorso then
        for _, child in ipairs(upperTorso:GetChildren()) do
            if child:IsA("Motor6D") and (child.Name:lower():find("waist") or child.Part1 == lowerTorso) then
                return child
            end
        end
    end
    for _, obj in ipairs(character:GetDescendants()) do
        if obj:IsA("Motor6D") and obj.Name:lower():find("waist") then return obj end
    end
    return nil
end
...
local function getYawBase()
    if AntiAim.AtTargets then
        ...
        if ef then
            ...
            if closest then
                return CFrame.lookAt(myRoot.Position, closest.Position)
            end
        end
    end
    return nil
...
-- 计算当前应该应用的角度
local function calculateAntiAimAngles()
    -- Pitch
    local pitchAngle = 0
    ...
    elseif AntiAim.Pitch == "半下" then pitchAngle = math.rad(45)
    elseif AntiAim.Pitch == "随机" then pitchAngle = math.rad(math.random(-89, 89))
    end
    ...
    elseif AntiAim.Yaw == "抖动" then
        local now = tick()
        if now - AntiAim._lastJitterTick >= (1 / math.max(AntiAim.JitterSpeed, 1)) then
    ...
    elseif AntiAim.Yaw == "随机" then yawOffset = math.random(-180, 180)
    end
    ...
    if AntiAim.Desync then
        if AntiAim.DesyncJitter then
            local jitterAdd = AntiAim.DesyncJitterRange > 0 and (AntiAim._jitterState and AntiAim.DesyncJitterRange or -AntiAim.DesyncJitterRange) or 0
    ...
    return pitchAngle, yawOffset, desyncAngle
end

-- ========== 初始化系统 ==========

local function initAntiAimObjects(char)
    -- 清理旧对象
    if AntiAim._bodyGyro then pcall(function() AntiAim._bodyGyro:Destroy() end); AntiAim._bodyGyro = nil end
    
    -- 查找所有 Motor6D
    AntiAim._neckMotor = findNeckMotor6D(char)
    AntiAim._rootMotor = findRootMotor6D(char)
    AntiAim._waistMotor = findWaistMotor6D(char)
    
    -- 保存原始值
    if AntiAim._neckMotor then
        AntiAim._originalNeckC0 = AntiAim._neckMotor.C0
        AntiAim._originalNeckC1 = AntiAim._neckMotor.C1
    end
    if AntiAim._rootMotor then
        AntiAim._originalRootC0 = AntiAim._rootMotor.C0
    end
    if AntiAim._waistMotor then
        AntiAim._originalWaistC0 = AntiAim._waistMotor.C0
    end
    
    -- 保存原始 HipHeight
    local hum = char:FindFirstChildOfClass("Humanoid")
    if hum then
        AntiAim._originalHipHeight = hum.HipHeight
    end
    
    -- 创建 BodyGyro (用于物理强制旋转)
    local hrp = char:FindFirstChild("HumanoidRootPart")
    if hrp then
        AntiAim._bodyGyro = Instance.new("BodyGyro")
        AntiAim._bodyGyro.Name = "__AntiAimGyro"
        AntiAim._bodyGyro.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
        AntiAim._bodyGyro.P = 1e6
        AntiAim._bodyGyro.D = 1e4
        AntiAim._bodyGyro.Parent = hrp
    end
    
    AntiAim._lastChar = char
    AntiAim._initialized = true
end
...
local function fullCleanup()
    ...
    if char and AntiAim._initialized then
        if AntiAim._neckMotor and AntiAim._neckMotor.Parent and AntiAim._originalNeckC0 then
            pcall(function() AntiAim._neckMotor.C0 = AntiAim._originalNeckC0 end)
            pcall(function() AntiAim._neckMotor.C1 = AntiAim._originalNeckC1 end)
        end
        if AntiAim._rootMotor and AntiAim._rootMotor.Parent and AntiAim._originalRootC0 then
            pcall(function() AntiAim._rootMotor.C0 = AntiAim._originalRootC0 end)
        end
        if AntiAim._waistMotor and AntiAim._waistMotor.Parent and AntiAim._originalWaistC0 then
            pcall(function() AntiAim._waistMotor.C0 = AntiAim._originalWaistC0 end)
        end
        
        -- 恢复 HipHeight
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then hum.HipHeight = AntiAim._originalHipHeight end
    end
    
    -- 销毁 BodyGyro
    if AntiAim._bodyGyro then
        pcall(function() AntiAim._bodyGyro:Destroy() end)
        AntiAim._bodyGyro = nil
    end
    
    -- 重置状态
    AntiAim._initialized = false
    AntiAim._lastChar = nil
    AntiAim._neckMotor = nil
    AntiAim._rootMotor = nil
    AntiAim._waistMotor = nil
    AntiAim._originalNeckC0 = nil
    AntiAim._originalNeckC1 = nil
    AntiAim._originalRootC0 = nil
    AntiAim._originalWaistC0 = nil
    AntiAim._fakeLagTicks = 0
    AntiAim._fakeLagStored = {}
end

-- ========== 核心 Anti-Aim 应用函数 (完全重写) ==========
...
local function applyAntiAim()
    ...
    -- 角色变化时重新初始化
    if char ~= AntiAim._lastChar or not AntiAim._initialized then
        initAntiAimObjects(char)
    end
    
    -- 重新查找 Motor6D (每帧确认它们还存在)
    if not AntiAim._neckMotor or not AntiAim._neckMotor.Parent then
        AntiAim._neckMotor = findNeckMotor6D(char)
        if AntiAim._neckMotor then
            AntiAim._originalNeckC0 = AntiAim._neckMotor.C0
            AntiAim._originalNeckC1 = AntiAim._neckMotor.C1
    ...
    if not AntiAim._rootMotor or not AntiAim._rootMotor.Parent then
        AntiAim._rootMotor = findRootMotor6D(char)
        if AntiAim._rootMotor then AntiAim._originalRootC0 = AntiAim._rootMotor.C0 end
    end
    if not AntiAim._waistMotor or not AntiAim._waistMotor.Parent then
        AntiAim._waistMotor = findWaistMotor6D(char)
        if AntiAim._waistMotor then AntiAim._originalWaistC0 = AntiAim._waistMotor.C0 end
    end

    local pitchAngle, yawOffset, desyncAngle = calculateAntiAimAngles()

    -- ===== 方法1: HumanoidRootPart CFrame 直接旋转 (最可靠) =====
    local baseCF = AntiAim._originalRootCFrame or hrp.CFrame
    local atTargetCF = getYawBase()
    
    local currentLook = atTargetCF and atTargetCF.LookVector or baseCF.LookVector
    local yawRad = math.rad(yawOffset)
    
    -- 绕Y轴旋转
    local rotatedLook = CFrame.Angles(0, yawRad, 0):VectorToWorldSpace(currentLook)
    local targetCF = CFrame.lookAt(hrp.Position, hrp.Position + rotatedLook)
    
    -- 应用Pitch (上下看)
    targetCF = targetCF * CFrame.Angles(pitchAngle, 0, 0)
    
    -- 强制设置 HRP CFrame (关键！这不会被动画覆盖因为我们在RenderSteped最后执行)
    hrp.CFrame = targetCF
    AntiAim._originalRootCFrame = targetCF

    -- ===== 方法2: BodyGyro 物理强制 =====
    if AntiAim._bodyGyro and AntiAim._bodyGyro.Parent then
        AntiAim._bodyGyro.CFrame = targetCF
    end

    -- ===== 方法3: Neck Motor6D 头部偏移 =====
    if AntiAim._neckMotor and AntiAim._neckMotor.Parent then
        local origC0 = AntiAim._originalNeckC0 or AntiAim._neckMotor.C0
        local origC1 = AntiAim._originalNeckC1 or AntiAim._neckMotor.C1
        -- 同时修改 C0 和 C1 以确保生效
        local neckOffset = CFrame.Angles(pitchAngle * 0.5, math.rad(desyncAngle), 0)
        AntiAim._neckMotor.C0 = origC0 * neckOffset
        AntiAim._neckMotor.C1 = origC1 * CFrame.Angles(0, math.rad(desyncAngle * 0.3), 0)
    end

    -- ===== 方法4: Root/Hip Motor6D 身体分离(Desync) =====
    if AntiAim.Desync then
        if AntiAim._rootMotor and AntiAim._rootMotor.Parent then
            local origRC0 = AntiAim._originalRootC0 or AntiAim._rootMotor.C0
            AntiAim._rootMotor.C0 = origRC0 * CFrame.Angles(0, math.rad(desyncAngle * 0.7), 0)
        end
        if AntiAim._waistMotor and AntiAim._waistMotor.Parent then
            local origWC0 = AntiAim._originalWaistC0 or AntiAim._waistMotor.C0
            AntiAim._waistMotor.C0 = origWC0 * CFrame.Angles(0, math.rad(desyncAngle * 0.5), 0)
        end
    end
    ...
    if AntiAim.FakeDuck then
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then hum.HipHeight = 0.5 end
    end
end
...
local function setupAntiAimLoop()
    ...
    -- 使用 RenderStepped 并设置高优先级
    antiAimConn = RunService.RenderStepped:Connect(function()
        if AntiAim.Enabled and isAlive() then
            local shouldRun = (AntiAim.Mode == "自动") or (AntiAim.Mode == "热键" and AntiAim.KeyHeld)
            if shouldRun then
                applyAntiAim()
                if AntiAim.FakeLag then updateFakeLag() end
            else
                -- 不运行时部分恢复
                local char = player.Character
                if char and AntiAim._initialized then
                    if AntiAim._bodyGyro and AntiAim._bodyGyro.Parent then
                        AntiAim._bodyGyro.CFrame = CFrame.new()
                    end
                    local hum = char:FindFirstChildOfClass("Humanoid")
                    if hum then hum.HipHeight = AntiAim._originalHipHeight end
                end
...
AntiAimGroup:AddToggle("AntiAimToggle", { Text = "启用反瞄准", Default = false, Callback = function(v) 
    ...
    if not v then 
        ...
        if player.Character and isAlive() then
            initAntiAimObjects(player.Character)
        end
        setupAntiAimLoop()
    end
end })
...
local function createNotif(text, textColor, bgColor, bgTrans)
    ...
    frame.Size = UDim2.new(0, textSize.X + 20*HitNotim.Scale, 0, textSize.Y + 12*HitNotif.Scale)
            ...
            for _, enemy in next, folder:GetChildren() do
                ...
                if hum and hum.Health > 0 then
                    ...
                    if part then
                        ...
                        if dist < minDist then minDist = dist; nearest = {pos = part.Position, part = part, model = enemy, hum = hum} end
                    end
                end
            end
        end
...
task.spawn(function()
    while task.wait(0.12) do
        if Wallbang.Enabled and wallCurrentWeapon and isWallbangWeapon(wallCurrentWeapon) then
            ...
            if prop and cur then local max = rawget(prop, "Rounds"); if max and cur < max then wallReload() end end
        end
    end
end)
...
LockGroup:AddDropdown("TargetSelectorDropdown", { Text = "锁定目标", Default = "无", Values = {"无"}, Callback = function(v) lockedTargetCharacter = v~="无" and target_map[v] or nil end })
LockGroup:AddButton("刷新列表", function()
    target_map = {}; local names = {"无"}
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p~=player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                if not (SilentAimSettings.TeamCheck and not IsEnemy(p.Character)) then
                    table.insert(names, p.Name); target_map[p.Name]=p.Character
                end
            end
        end
    end
    if SilentAimSettings.TargetMode=="NPC" or SilentAimSettings.TargetMode=="所有" then
        for _, npc in ipairs(npcList) do
            if npc and npc.Name and npc.PrimaryPart then table.insert(names, npc.Name); target_map[npc.Name]=npc end
        end
    end
...
local function updateSkeleton(esp,model)
    ...
    local connections={{"Head","UpperTorso"},{"UpperTorso","LowerTorso"},{"UpperTorso","LeftUpperArm"},{"LeftUpperArm","LeftLowerArm"},{"LeftLowerArm_LeftHand"},{"UpperTorso","RightUpperArm"},{"RightUpperArm_RightLowerArm"},{"RightLowerArm_RightHand"},{"LowerTorso_LeftUpperLeg"},{"LeftUpperLeg_LeftLowerLeg"},{"LeftLowerLeg_LeftFoot"},{"LowerTorso_RightUpperLeg"},{"RightUpperLeg_RightLowerLeg"},{"RightLowerLeg_RightFoot"}}
...
RunService.RenderStepped:Connect(function()
    ...
    for _,enemy in ipairs(ef:GetChildren()) do
        ...
        if hum and hum.Health>0 and root and head then
            ...
            if rootOn then
                ...
                if Esp.Skeleton then updateSkeleton(esp,enemy) else for_,l in pairs(esp.bones) do l.Visible=false end end
            else
                esp.boxOutline.Visible=false; esp.box.Visible=false; esp.name.Visible=false; esp.distance.Visible=false; esp.healthOutline.Visible=false; esp.healthBar.Visible=false for_,l in pairs(esp.bones) do l.Visible=false end
            end
        end
    end
    for enemy,esp in pairs(espCache) do
        if not curAlive[enemy] then
            ...
            for_,l in pairs(esp.bones) do l:Remove() end
            espCache[enemy]=nil
        end
    end
end)
...
Library:OnUnload(function()
    ...
    pcall(function() fullCleanup() end)
