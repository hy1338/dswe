...
UserInputService.InputBegan:Connect(function(i)
    if i.UserInputType == SpeedHack.Key or i.KeyCode == SpeedHack.Key then SpeedHack.KeyHeld = true end
end)
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType == SpeedHack.Key or i.KeyCode == SpeedHack.Key then SpeedHack.KeyHeld = false end
end)

local function applySpeedHack()
    if not SpeedHack.Enabled or not isAlive() then return end
    ...
    -- 保存原始速度
    if SpeedHack._originalWalkSpeed == nil then
        SpeedHack._originalWalkSpeed = hum.WalkSpeed
    end

    if SpeedHack.Mode == "WalkSpeed" then
        hum.WalkSpeed = SpeedHack.Speed
    elseif SpeedHack.Mode == "Velocity" then
        ...
        if moveDir.Magnitude > 0 then
            moveDir = moveDir.Unit * SpeedHack.Speed
            hrp.Velocity = Vector3.new(moveDir.X, hrp.Velocity.Y, moveDir.Z)
        end
    elseif SpeedHack.Mode == "CFrame" then
        local moveDir = Vector3.new()
        if UserInputService:IsKeyDown(Enum.KeyCode.W) then moveDir = moveDir + camera.CFrame.LookVector end
        if UserInputService:IsKeyDown(Enum.KeyCode.S) then moveDir = moveDir - camera.CFrame.LookVector end
        if UserInputService:IsKeyDown(Enum.KeyCode.A) then moveDir = moveDir - camera.CFrame.RightVector end
        ...
        if moveDir.Magnitude > 0 then
            hrp.CFrame = hrp.CFrame + (moveDir.Unit * SpeedHack.Speed * 0.1)
        end
    end
end
...
local function cleanupSpeedHack()
    ...
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    if hum and SpeedHack._originalWalkSpeed then
        hum.WalkSpeed = SpeedHack._originalWalkSpeed
    end
    SpeedHack._originalWalkSpeed = nil
end

-- 使用 Heartbeat 确保流畅
SpeedHack._connection = RunService.Heartbeat:Connect(function()
    if SpeedHack.Enabled and isAlive() then
        applySpeedHack()
    else
        if SpeedHack._originalWalkSpeed then
            cleanupSpeedHack()
        end
    end
end)
...
player.CharacterAdded:Connect(function()
    ...
    if SpeedHack.Enabled then
        SpeedHack._originalWalkSpeed = 16
    end
end)

-- SpeedHack UI
local SpeedHackGroup = Tabs.Combat:AddRightGroupbox("速度修改", "activity")
SpeedHackGroup:AddToggle("SpeedHackToggle", { Text = "启用加速", Default = false, Callback = function(v) 
    SpeedHack.Enabled = v
    if not v then 
        cleanupSpeedHack() 
    end
end })
SpeedHackGroup:AddDropdown("SpeedHackMode", { Text = "加速模式", Values = {"WalkSpeed","Velocity","CFrame"}, Default = "WalkSpeed", Callback = function(v) SpeedHack.Mode = v end })
SpeedHackGroup:AddSlider("SpeedHackSpeed", { Text = "速度", Default = 30, Min = 16, Max = 100, Rounding = 0, Suffix = "", Callback = function(v) SpeedHack.Speed = v end })
SpeedHackGroup:AddDropdown("SpeedHackHotkey", { Text = "热键", Values = hotkeyValues, Default = getHotkeyName(SpeedHack.Key), Callback = function(v) SpeedHack.Key = hotkeyMap[v] or Enum.KeyCode.LeftShift end })
...
Library:OnUnload(function()
    ...
    pcall(function() cleanupSpeedHack() end)
